...
def get_model( shape_input , dense_unit = 25 ):

  model = Sequential()

  model.add(Convolution2D(20, (3, 3), activation='relu', padding='same', input_shape=shape_input))
  model.add(BatchNormalization())
  model.add(MaxPooling2D(2, 2))
  model.add(Dropout(0.50))

  model.add(Convolution2D(50, (3, 3), activation='relu', padding='same', input_shape = shape_input ))
  model.add(BatchNormalization())
  model.add(MaxPooling2D(2, 2))
  model.add(Dropout(0.50))
